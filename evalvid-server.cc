...
EvalvidServer::Setup()
{
	...
	while (videoTraceFile >> frameId >> frameType >> frameSize >> numOfTcpPackets >> sendTime)
	{
		...
		//NS_LOG_LOGIC(">> EvalvidServer: " << frameId << "\t" << frameType << "\t" <<frameSize << "\t" << numOfTcpPackets << "\t" << sendTime);
...
void
EvalvidServer::Send () /*Sends one frame at a time! Calls itself until the last frame of the segment is sent.*/
{
  ...
  if (m_videoInfoMapIt != m_videoInfoMap.end())
    {
      for(int i=0; i<m_videoInfoMapIt->second->numOfTcpPackets; i++)
        {
          ...
          packetcount++;
          std::cout <<"Server: bytes sent->"<<p->GetSize() <<" frame size->" <<mpeg_header.GetSize() <<" packetcount:"<<packetcount <<std::endl;
      ...
      m_senderTraceFile << std::fixed << std::setprecision(4) << Simulator::Now().ToDouble(Time::S)
                        << std::setfill(' ') << std::setw(16) <<  "id " << m_packetId
                        << std::setfill(' ') <<  std::setw(16) <<  "tcp " << p->GetSize()
                        << std::setfill(' ') << std::setw(16) <<  "frame id " <<m_videoInfoMapIt->first
                        << std::endl;
      ...
      {
    	  NS_LOG_INFO(">> EvalvidServer: Sending segment " <<segmentId <<" complete!");
      }
      else
        {
