...
/*	Create a P2P connection between the P-GW and our remote host. Assign IP addresses.*/
...
/*	Create a mobility model and install our nodes in it. (Initial position is defined by ListPositionAllocator.)*/
...
/*	Create Client applications. Install on our UE nodes.*/
	for (uint16_t i = 0; i < numberOfNodes; i++)
	{
		EvalvidClientHelper client (internetIpIfaces.GetAddress (1),serverPort);//serverIP,serverPort
		std::ostringstream ReceiverDumpFilename;
		ReceiverDumpFilename <<"rd_a0" <<i <<"_lte";
		client.SetAttribute ("ReceiverDumpFilename", StringValue(ReceiverDumpFilename.str()));
		client.SetAttribute ("VideoId", UintegerValue(1));
		apps = client.Install (ueNodes.Get(i));
		apps.Start (Seconds (1.1));
		apps.Stop (Seconds (9.9)); //Note: Clients start after and finish before the Server.
	}
