...
namespace ns3 {
    ...
    EvalvidClient::GetTypeId (void)
    {
        static TypeId tid = TypeId ("ns3::EvalvidClient")
                            ...
                            .AddAttribute ("PacketPayload",
                                          ...
                                          /*In our case:	MTU	 - (SEQ_HEADER + TCP_HEADER + IP_HEADER + HTTP HEADER + MPEG HEADER)
                                           * 		so:		1500 - (12	+	20	+	20	+	28	+	32)	= 1388
    ...
    EvalvidClient::MessageReceived(Packet message)
    {
        ...
        m_player.ReceiveFrame(&message); //TODO: In case frame consists of more than 1 packets, play them all together.
        ...
        receiverDumpFile << std::fixed << std::setprecision(4) << Simulator::Now().ToDouble(ns3::Time::S)
                                       << std::setfill(' ') << std::setw(16) <<  "id " << seqTs.GetSeq()
                                       << std::setfill(' ') <<  std::setw(16) <<  "tcp " << message.GetSize()
                                       << std::endl;
        ...
        if (mpegHeader.GetFrameId()!= 0 && mpegHeader.GetFrameId() % MPEG_FRAMES_PER_SEGMENT == 0)
        {
            ...
            {
                m_player.SchduleBufferWakeup(bufferDelay, this); //if we want to schedule a segment request after delaying the buffer.
            }
    ...
    void
    EvalvidClient::CalcNextSegment(uint32_t currRate, uint32_t & nextRate, Time & delay)
    {
        nextRate = currRate;
        delay = Seconds(0);
    }
